...
class RedirectUrl(object):
    def __init__(self, s):
        with closing(
            s.get(
                "https://get.cbord.com/tcu/full/login.php", timeout=TIMEOUT
            )
        ) as r:
            r.raise_for_status()
            redirect_url = BeautifulSoup(r.text, PARSER).select_one(
...
class Table(object):
    def __init__(self, s):
        ...
        self.html = BeautifulSoup(response.text, PARSER).find("table").find_all("tr")[1:]
...
if __name__ == "__main__":
    with requests.Session() as session:
        ...
        print(Table(session).get_balance('Meal Plan Frog Bucks'))  # Get an account balance
